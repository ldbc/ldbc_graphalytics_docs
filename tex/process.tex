...
\begin{itemize}
    \item \textbf{[A] Format-graph}: The benchmark suite minimizes the ``input data" into ``formatted dataset" (See more in \sref{sec:process:execution:run_flow}) by removing unused vertex and edge properties.
    ...
    \item \textbf{[C] Execute-run}: The platform executes a benchmark run with a specific algorithm and dataset (See more details in \sref{sec:process:execution:run_flow}). All benchmark runs using the same input dataset can use the prepared graph dataset during the ``load-graph` step. 
